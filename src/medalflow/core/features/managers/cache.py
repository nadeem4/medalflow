...
class CacheManager(CacheProtocol, FeatureManager):
    ...
    def clear(self, pattern: str = "*") -> int:
        ...
        # Clear by pattern
        keys_to_delete = [key for key in self._storage if fnmatch.fnmatch(key, pattern)]
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        """
        if not self._initialized:
            return {"enabled": self.is_available(), "initialized": False, "total_keys": 0}

        # Get top 10 most accessed keys
        top_accessed = sorted(self._access_count.items(), key=lambda x: x[1], reverse=True)[:10]
