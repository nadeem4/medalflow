...
The `medalflow.logging` package supplies a structured logging foundation for the MedalFlow ETL framework. It emits JSON logs, enriches them with request and trace metadata, and stays sink-agnostic so host applications decide where records are delivered.
...
```python
from medalflow.logging import setup_logging
...
```python
from medalflow.logging import get_logger
from medalflow.logging.filters import set_request_context
...
```json
{
  ...
  "level": "INFO",
  "logger": "medalflow.compute.engines.synapse",
